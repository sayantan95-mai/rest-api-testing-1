...
class TestBookingWorkflow:
    ...
    def test_create_booking(self, booking_helper: BookingHelper, sample_booking_data):
        ...
        print(f"Created booking ID: {self.created_booking_id}") # For visibility during test run

    def test_get_booking(self, booking_helper: BookingHelper, sample_booking_data):
        """Tests retrieving the created booking."""
        assert TestBookingWorkflow.created_booking_id is not None, "Booking ID not created"
    ...
    def test_update_booking(self, booking_helper: BookingHelper, authenticated_api_client):
    ...
    def test_partial_update_booking(self, booking_helper: BookingHelper, authenticated_api_client):
        """Tests partially updating the booking (requires auth)."""
        client, token = authenticated_api_client
        assert TestBookingWorkflow.created_booking_id is not None, "Booking ID not created"
        partial_data = {"firstname": "Pytest-Patched", "additionalneeds": "Late Checkout"}
    ...
    def test_delete_booking(self, booking_helper: BookingHelper, authenticated_api_client):
        """Tests deleting the created booking (requires auth)."""
        client, token = authenticated_api_client
        assert TestBookingWorkflow.created_booking_id is not None, "Booking ID not created"

        response = booking_helper.delete_booking(TestBookingWorkflow.created_booking_id, token)
        ...
        verify_response = booking_helper.get_booking(TestBookingWorkflow.created_booking_id)
...
def test_get_nonexistent_booking(booking_helper: BookingHelper):
    ...
    assert response.status_code == 404, f"Expected 404, got {response.status_code}. Body: {response.text}"
